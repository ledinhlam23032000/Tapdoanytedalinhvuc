...
### 60 ADR — `docs/architecture/DECISIONS.md:<dòng>`
...
`:1137` ADR-052 — Dữ liệu lâm sàng KHÔNG dùng application-level fiel…
`:1181` ADR-053 — Kiến trúc phân cấp AI: 1 Ecosystem AI/Ecosystem, 1…
`:1210` ADR-054 — Ba khái niệm tách biệt: AiConversation (hội thoại)…
`:1239` ADR-055 — Tool Registry là ranh giới an toàn cứng: chỉ Busin…
`:1273` ADR-056 — Risk không suy từ tên tool; mọi hành động high-ris…
`:1307` ADR-057 — AiActionProposal là entity Decision Inbox riêng; h…
`:1343` ADR-058 — Job/Run Engine là database-polling worker đơn giản…
`:1374` ADR-059 — Ma trận scope enforcement cross-company/cross-ecos…
`:1405` ADR-060 — Observe/Signal layer: code sinh fact xác định trướ…
